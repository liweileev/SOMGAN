...
from networks import ResNet_share_network, CNN_share_network
...
class Trainer(object):
    ...
    def train(self):
        ...
        for step in tnrange(start, self.total_step):
            ...
            ShareD_real = self.ShareD(real_images)
            for i in range(self.d_num):
                D_real_vals[i] = names['D_' + str(i)](ShareD_real)
            Weight = self.ComputeWeight(D_real_vals, step)
            with torch.no_grad():
                fake_images = self.G(z)
            ShareD_fake = self.ShareD(fake_images)
...
#                 di_loss_fake = - torch.log(names['D_' + str(i)](fake_images)).mean()
                di_loss_fake = torch.nn.ReLU()(1.0 + names['D_' + str(i)](ShareD_fake)).mean()
            ...
            fake_images = self.G(z)
            g_ShareD_fake = self.ShareD(fake_images)
            for i in range(self.d_num):
#                 gi_loss = torch.log(names['D_' + str(i)](fake_images)).mean()
                gi_loss = - names['D_' + str(i)](g_ShareD_fake).mean()
                g_loss += gi_loss
            ...
            if (step+1) % self.model_save_step==0:
                torch.save(self.ShareD.state_dict(), os.path.join(self.model_save_path, 'ShareD_step{}.pth'.format(step+1)))
                for i in range(self.d_num):
    ...
    def build_model(self):
        ...
        if self.network == 'ResNet_share_network':
            self.ShareD = ResNet_share_network.ShareDiscriminator(self.channel, self.d_conv_dim).cuda()
            for i in range(self.d_num):
                names['D_' + str(i)] = ResNet_share_network.Discriminator(self.imsize, self.d_conv_dim).cuda()
            self.G = ResNet_share_network.Generator(self.imsize, self.z_dim, self.channel, self.g_conv_dim).cuda()  
        elif self.network == 'CNN_share_network':
            self.ShareD = CNN_share_network.ShareDiscriminator(self.imsize, self.channel, self.d_conv_dim).cuda()
            for i in range(self.d_num):
                names['D_' + str(i)] = CNN_share_network.Discriminator(self.d_conv_dim).cuda()
            self.G = CNN_share_network.Generator(self.imsize, self.z_dim, self.channel, self.g_conv_dim).cuda()  
       ...
       # Loss and optimizer
        self.d_share_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.ShareD.parameters()), self.d_lr, [self.beta1, self.beta2])
        for i in range(self.d_num):
        ...
        if self.printnet:
            ...
            print("D_share summary:")
            summary(self.ShareD, (self.channel, self.imsize, self.imsize))  
            print("D_0 summary:")    
            if self.network == 'ResNet_share_network':
                summary(self.D_0, (self.d_conv_dim, int(self.imsize/4), int(self.imsize/4)))        
            elif self.network == 'CNN_share_network':
                summary(self.D_0, (self.d_conv_dim * 4, 8, 8))        
            # self.G.cuda()
            # self.D_0.cuda()
            print("\n=============================\n")
    ...
    def load_pretrained_model(self):
        names = self.__dict__
        self.ShareD.load_state_dict(torch.load(os.path.join(self.model_save_path, 'ShareD_step{}.pth'.format(self.pretrained_model))))
        for i in range(self.d_num):
    ...
    def reset_grad(self):
        names = self.__dict__
        self.d_share_optimizer.zero_grad()
        for i in range(self.d_num):
